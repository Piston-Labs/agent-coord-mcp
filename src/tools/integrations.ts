...
export function registerIntegrationTools(server: McpServer) {
  ...
  server.tool(
    'notion',
    'Notion integration for knowledge base and documentation. Search pages, get content, query databases.',
    {
      action: z.enum(['search', 'page', 'database', 'query', 'databases']).describe('search=find pages, page=get content, database=get db info, query=query db entries, databases=list all dbs'),
      pageId: z.string().optional().describe('Page ID (required for page action)'),
      databaseId: z.string().optional().describe('Database ID (required for database/query actions)'),
      query: z.string().optional().describe('Search query for finding pages'),
      limit: z.number().optional().describe('Max results to return (default 25, max 100)'),
      agentId: z.string().describe('Your agent ID')
    },
    async (args) => {
      const { action, pageId, databaseId, query, limit, agentId } = args;
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/api/notion?${params}`);
        const data = await res.json();

        // Format search results nicely
        if (action === 'search' && data.results) {
          const lines = [
            `## Notion Search Results`,
            ``,
            `Found **${data.count}** results${query ? ` for "${query}"` : ''}:`,
            ``
          ];

          for (const item of data.results) {
            lines.push(`- **${item.title}** (${item.type})`);
            lines.push(`  ID: ${item.id} | Updated: ${item.lastEditedTime}`);
          }

          if (data._note) {
            lines.push('', `_${data._note}_`);
          }

          return { content: [{ type: 'text', text: lines.join('\n') }] };
        }

        // Format page content nicely
        if (action === 'page' && data.content) {
          const lines = [
            `## ${data.title || 'Untitled'}`,
            ``,
            `**URL:** ${data.url}`,
            `**Last Edited:** ${data.lastEditedTime}`,
            ``,
            `### Content:`
          ];
        ...
        // Format databases list nicely
        if (action === 'databases' && data.databases) {
          const lines = [
            `## Notion Databases`,
            ``,
            `Found **${data.count}** databases:`,
            ``
          ];

          for (const db of data.databases) {
            lines.push(`- **${db.title}**`);
            lines.push(`  ID: ${db.id} | Updated: ${db.lastEditedTime}`);
          }
