...
const shopMapping = {
  ...
  'Document': 'PDF Upload & Processing',
...
// Process based on command line arg
const product = process.argv[2];
const inputFile = process.argv[3];

if (!product || !inputFile) {
  console.log('Usage: node map-features.js <consumer|shop|carteldb> <input.json>');
  process.exit(1);
}
...
data.features.forEach(f => {
  const match = mapFeature(f, mapping, components);
  if (match) {
    results.push({
      featureId: f.id,
      name: f.name,
    ...
    console.error('UNMAPPED:', f.id, f.name);
  }
});
